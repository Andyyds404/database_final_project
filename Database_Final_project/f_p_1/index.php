...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    if ($stmt->fetch()) {
        ...
        echo '<script>  alert("帳號已存在");   </script>'; // null -> echo '<script>  alert("帳號已存在");   </script>';
    } else {
        // 插入 User 表
        $stmt = $pdo->prepare("INSERT INTO User (Account, Password, Name) VALUES (?, ?, ?)"); // , Email) VALUES (?, ?, ?, ?)"); -> ) VALUES (?, ?, ?)");
        $stmt->execute([$account, $password, $name]); // 實際應用應使用 password_hash() // , $email]); -> ]);
        ...
        echo '<script>  alert("註冊成功，請登入");   </script>'; // null -> echo '<script>  alert("註冊成功，請登入");   </script>';
    }
}
?>
<!DOCTYPE html>
